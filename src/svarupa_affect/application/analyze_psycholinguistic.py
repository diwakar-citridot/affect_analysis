"""PsycholinguisticLayer (use case): LLM-primary psycholinguistic analysis (PSY v1).

Mirrors the AFF ``AffectLayer.analyze_full`` LLM-primary path: one Bedrock call
produces linguistic-form features plus a ``DimensionalSignal[]`` fusion envelope.
"""
...
from .psycholinguistic_orchestrator import (
    LAYER_CODE,
    PsycholinguisticOrchestrator,
    PsycholinguisticResult,
)

logger = logging.getLogger("svarupa_affect.psycholinguistic")

_RELEVANCE_FLOOR = 0.12


@dataclass(frozen=True)
class PsycholinguisticAnalyzeResult:
    signals: list[DimensionalSignal]
    psycholinguistic: PsycholinguisticResult
    used: bool
    attempted: bool
    abstained: bool
    failure: str | None
...
class PsycholinguisticLayer:
    ...
    async def analyze_full(self, ctx: LayerContext) -> PsycholinguisticAnalyzeResult:
        ...
        return PsycholinguisticAnalyzeResult(
            signals=signals,
    ...
    def _provenance(self, result: PsycholinguisticResult) -> Provenance:
...
def _build_primary_provider(settings: Settings) -> object:
    ...
    try:
        ...
        logger.info(
            "PSY LLM-primary enabled (Bedrock model_id=%s, timeout=%.0fs)",
        ...
        if settings.llm_strict:
            raise ModelUnavailable(
                f"PSY llm_primary but Bedrock could not initialize: {exc}"
            ) from exc
        logger.error(
            "PSY LLM-primary provider failed (%s: %s); will abstain on failure",
            type(exc).__name__,
            exc,
        )
        return NullLLMProvider()
...
def build_default_psycholinguistic_layer() -> PsycholinguisticLayer:
    """Wire the psycholinguistic orchestrator into the use case (DI composition root)."""
    ...
    from ..infrastructure.kg.triplet_registry import build_psycholinguistic_triplet_vocabulary
