"""LLM-primary narrative-arc orchestration (NAR v1).

One Bedrock call reads temporal structure and scores D10/D12/D13/D14/D16/D29 onto
closed KG vocabulary. Failures degrade to abstention — never fabricate signals.
"""
...
from ..infrastructure.llm.prompts import narrative_arc_v1 as prompt_mod
...
logger = logging.getLogger("svarupa_affect.narrative_primary")
...
class NarrativeArcResult:
    ...
    narrative_arc: NarrativeArcReading
...
class NarrativeArcOrchestrator:
    def __init__(
        self,
    ...
    async def score(self, ctx: LayerContext) -> NarrativeArcResult:
        neutral = self._neutral_result(reasons=["orchestrator_disabled"])
        ...
        logger.info(
            "NAR-primary starting request_id=%s prompt_version=%s model_id=%s samples=%d",
            ctx.request_id,
            self.prompt_version,
            self.model_id,
            n,
        ...
        narrative_arc = self._narrative_arc_from_payload(merged)
    ...
    def build_uncertainty(self, result: NarrativeArcResult, text: str, n_segments: int = 1):
        length_factor = clip(len(tokenize(text)) / 25.0)
        snapshot_cap = 0.25 if result.narrative_arc.single_snapshot else 1.0
        return build_uncertainty_profile(
            field=result.field,
            evidence_strength=result.field.uncertainty.evidence_quality.value * snapshot_cap,
                ...
                try:
                    ...
                    if attempt > 0 and last_error:
                        attempt_prompt = (
                            ...
                            "Return corrected JSON with narrative_arc as an object."
                        )
                    ...
                    return (
                        prompt_mod.validate_narrative_arc(raw, self._emit_dimensions),
                        sample_usage,
                    )
                except (prompt_mod.NarrativeArcValidationError, ModelUnavailable) as exc:
                    _add_usage(sample_usage, metrics)
                    last_error = str(exc)
                    logger.warning(
                        "NAR-primary invalid payload (attempt %d/3): %s", attempt + 1, exc
                    )
    ...
    @staticmethod
    def _narrative_arc_from_payload(merged: dict) -> NarrativeArcReading:
        arc = merged.get("narrative_arc", {})
        if not isinstance(arc, dict):
            arc = {}
        events = arc.get("events", [])
        if not isinstance(events, list):
            events = []
        return NarrativeArcReading(
            shape=str(arc.get("shape", "none")),
            loop_detected=bool(arc.get("loop_detected", False)),
            single_snapshot=bool(arc.get("single_snapshot", True)),
            temporal_structure=str(arc.get("temporal_structure", "")),
            mismatch_detected=bool(arc.get("mismatch_detected", False)),
            events=tuple(str(e) for e in events[:8]),
            rationale=str(arc.get("rationale", "")),
        )
