...
Steps:
  1. svarupa_dimension          -> svarupa_dimensions
  2. svarupa_concept (deduped)    -> svarupa_concepts
  3. DDL on svarupa_concepts: drop physical/emotional/mental if present; add perspective
  4. svarupa_concept rows         -> svarupa_concept_descriptions
     (perspective, status -> status_id, description_new -> description)
...
Run from repo root::
    ...
    PYTHONPATH=src python scripts/migrate_companion_to_assistant.py
    PYTHONPATH=src python scripts/migrate_companion_to_assistant.py --dry-run
"""
...
# Companion dimension slug -> target dimension_id (merged / renamed slugs not in svarupa_dimension).
...
def build_dimension_slug_map(
    ...
    with companion.cursor() as cur:
        cur.execute("SELECT id, dimension FROM svarupa_dimension")
        for row in cur.fetchall():
            target_id = COMPANION_DIM_TO_TARGET.get(int(row["id"]))
...
def migrate_dimensions(
    ...
    *,
    ...
    with companion.cursor() as cur:
        cur.execute("SELECT id, dimension FROM svarupa_dimension ORDER BY id")
    ...
    with master.cursor() as cur:
        for row in source_rows:
            ...
            target_id = COMPANION_DIM_TO_TARGET.get(companion_id)
            if target_id is None:
                logger.warning("Skipping unmapped companion dimension id=%s", companion_id)
                continue
...
def group_source_concepts(
    ...
    for row in rows:
        ...
        target_dim = COMPANION_DIM_TO_TARGET.get(companion_dim)
        if target_dim is None:
            continue
...
def migrate_concept_descriptions(
    ...
    *,
    ...
    for row in source_rows:
        params, _skip_reason = _concept_description_params(row, concept_id_map, status_map)
        if params is None:
            skipped += 1
            continue
...
def run_migration(*, dry_run: bool) -> None:
    ...
    try:
        ...
        dimension_slug_map = build_dimension_slug_map(companion, master)

        dim_count = migrate_dimensions(companion, master, dry_run=dry_run)
        ...
        grouped = group_source_concepts(source_rows)
        logger.info(
